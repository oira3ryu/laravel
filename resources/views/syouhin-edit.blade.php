    ...
    @endif
      <form method="post" action="{{ route('syouhins.update', $syouhin->id) }}">
          ...
          <div class="form-group">
              ...
              <select name="syouhin_syubetsu_id" class="form-control">
                  ...
                  @foreach ($syouhin_syubetsu as $syouhin_syubetsus)
                      <option class="{{ $syouhin_syubetsus->id }}" class1="{{ $syouhin_syubetsus->id }}" value="{{ old('id', $syouhin_syubetsus->id) }}" 
                          @if($syouhin_syubetsus->id==$syouhin->syouhin_syubetsu_id || $syouhin_syubetsus->id === (int)old('id')) selected @endif>{{ $syouhin_syubetsus->meisyou }}
                      </option>
                  @endforeach
              </select>
              ...
              <select name="hyouji" class="form-control">
                  ...
                  @foreach ($hyouji as $hyoujis)
                      <option class="{{ $hyoujis->id }}" value="{{ old('id', $hyoujis->id) }}" 
                          @if($hyoujis->id==$syouhin->hyouji || $hyoujis->id === (int)old('id')) selected @endif>{{ $hyoujis->meisyou }}
                      </option>
                  @endforeach
              </select>
              @error('hyouji')
          ...
          <div class="form-group">
              ...
              <input type="text" class="form-control" name="narabikae" value="{{ $syouhin->narabikae }}"/>
          </div>
          <div class="form-group">
