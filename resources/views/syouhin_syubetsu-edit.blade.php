    ...
    @endif
      <form method="post" action="{{ route('syouhin_syubetsus.update', $syouhin_syubetsu->id) }}">
              ...
              <select name="hyouji" class="form-control">
                  ...
                  @foreach ($hyouji as $hyoujis)
                      <option class="{{ $hyoujis->id }}" value="{{ old('id', $hyoujis->id) }}" 
                          @if($hyoujis->id==$syouhin_syubetsu->hyouji || $hyoujis->id === (int)old('id')) selected @endif>{{ $hyoujis->meisyou }}
                      </option>
                  @endforeach
              </select>
              @error('hyouji')
          ...
          <div class="form-group">
              ...
              <input type="text" class="form-control" name="narabikae" value="{{ $syouhin_syubetsu->narabikae }}"/>
          </div>
          <div class="form-group">
