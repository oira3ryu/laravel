...
class CreateSyukkakanrisTable extends Migration
{
    ...
    public function up()
    {
	    ...
	    $table->date('hiduke'); // 日付
            ...
            $table->unsignedBigInteger('seihinsyubetsu_id'); // 製品種別
            $table->unsignedDecimal('suuryou', 4, 2); // 納入数量
            $table->unsignedDecimal('motikaerisuuryou', 4, 2); // 持帰り数量
            $table->unsignedDecimal('pumpkaisuu', 4, 2); // ポンプ車回数
	    $table->unsignedDecimal('pumpsuuryou', 4, 2); // ポンプ車打設数量
            ...
            $table->foreign('seihinsyubetsu_id')->references('id')->on('seihinsyubetsus')->onDelete('cascade');
